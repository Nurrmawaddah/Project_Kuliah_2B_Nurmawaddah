    ...
    if ($cek === false) {
        ...
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
            ...
            if (mysqli_num_rows($select) > 0) {
                ...
                $query_tb_barang = mysqli_query($conn, "INSERT INTO tb_barang (foto, nama_pupuk, keterangan, id_kategori, stock) VALUES ('$uploaded_filename','$nama_pupuk','$keterangan','$kategori','$stock')");

                // Ambil id_pupuk yang baru saja dimasukkan
                $id_pupuk = mysqli_insert_id($conn);

                // Insert data ke tb_masuk
                $query_tb_masuk = mysqli_query($conn, "INSERT INTO tb_masuk (id_pupuk, id_user, jumlah_masuk) VALUES ('$id_pupuk', '$id_user_sesi', '$stock')");
